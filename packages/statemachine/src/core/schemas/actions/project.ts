 ...
 *
 * Actions for updating GitHub Project fields (Status, Iteration, Failures)
 * and the block (circuit breaker) action.
 */
...
import { ProjectStatusSchema, type ProjectStatus } from "../entities.js";
import type { RunnerContext, Octokit } from "../../executor.js";
  ...
  updateProjectStatus: defAction(
    ...
    }),
    {
      ...
      execute: async (action, ctx) => {
        const { itemId, projectFields, currentState } =
          await getOrAddProjectItem(ctx.octokit, ctx, action.issueNumber);
  ...
  incrementIteration: defAction(
    ...
    }),
    {
      predict: (_a, target) => ({
        target: {
          iteration:
            (target && "iteration" in target ? target.iteration : 0) + 1,
        },
      }),
      execute: async (action, ctx) => {
        const { itemId, projectFields, currentState } =
  ...
  block: defAction(
    ...
    }),
    {
      ...
      execute: async (action, ctx) => {
        ...
        core.info(`Blocked issue #${action.issueNumber}: ${action.message}`);
        return { blocked: true };
      },
    },
  ),
