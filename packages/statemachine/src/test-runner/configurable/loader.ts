...
const DEFAULT_FIXTURES_BASE_PATH = ".github/actions-ts/sm-test-runner/fixtures";
 ...
 * @param basePath - Base path for fixtures (default: ".github/actions-ts/sm-test-runner/fixtures")
 * @returns Fully loaded scenario ready for execution
 * @throws Error if scenario or any fixture is invalid
 */
export async function loadScenario(
  scenarioName: string,
  basePath: string = DEFAULT_FIXTURES_BASE_PATH,
...
async function loadStateFixtures(statesDir: string): Promise<{
  ...
  for (const file of files) {
    ...
    const fixture: StateFixture = parseResult.data;
    const stateName: StateName = fixture.state;

    // Check for duplicate states
    if (fixtures.has(stateName)) {
      throw new Error(
        `Duplicate state '${stateName}' in ${file}. Each state can only appear once in a scenario.`,
      );
    }

    orderedStates.push(stateName);
    fixtures.set(stateName, fixture);
...
async function loadReferencedMocks(
  ...
  for (const [state, fixture] of fixtures.entries()) {
