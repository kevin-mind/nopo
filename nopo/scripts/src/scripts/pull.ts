...
import { requiresBuild, isPackageService } from "../config/index.ts";
...
export default class PullScript extends TargetScript {
  ...
  override async fn(args: ScriptArgs) {
    let requestedTargets = args.get<string[]>("targets");

    if (!requestedTargets || requestedTargets.length === 0) {
      // No targets specified - pull all buildable service images
      requestedTargets = this.runner.config.targets.filter((t) => {
        const service = this.runner.getService(t);
        return requiresBuild(service) && !isPackageService(service);
      });

      if (requestedTargets.length === 0) {
        this.log("No pullable services found");
        return;
      }
    }
