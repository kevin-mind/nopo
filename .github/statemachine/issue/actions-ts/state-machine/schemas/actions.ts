import { z } from "zod";
import { ProjectStatusSchema } from "./entities.js";
 ...
 */
export const AppendAgentNotesActionSchema = BaseActionSchema.extend({
  type: z.literal("appendAgentNotes"),
  issueNumber: z.number().int().positive(),
...
export type AppendAgentNotesAction = z.infer<
  typeof AppendAgentNotesActionSchema
>;
...
/**
 * Issue-specific action types
 * These actions are only used in the issue/PR automation context
 */
...
/**
 * Discussion-specific action types
 * These actions are only used in the discussion automation context
 */
...
/**
 * Type for actions that are specific to issue automation
 */
export type IssueAction = Extract<Action, { type: IssueActionType }>;

/**
 * Type for actions that are specific to discussion automation
 */
export type DiscussionAction = Extract<Action, { type: DiscussionActionType }>;

/**
 * Type for actions that are shared between both contexts
 */
