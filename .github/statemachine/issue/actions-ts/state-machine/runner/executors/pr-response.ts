...
export async function executeApplyPRResponseOutput(
  ...
  core.info(`Applying PR response output: had_commits=${responseOutput.had_commits}`);
  ...
  if (responseOutput.agent_notes && responseOutput.agent_notes.length > 0) {
    ...
    if (updatedBody !== currentBody) {
      ...
      core.info(`Appended ${responseOutput.agent_notes.length} agent notes to issue #${action.issueNumber}`);
    }
  }
